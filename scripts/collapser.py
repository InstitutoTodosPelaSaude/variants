...
if __name__ == '__main__':
    ...
    # path = '/Users/Anderson/Library/CloudStorage/GoogleDrive-anderson.brito@itps.org.br/Outros computadores/My Mac mini/google_drive/ITpS/projetos_itps/vigilanciagenomica/analyses/relatorioXX_20220812/results/'
    # input = path + 'matrix_casos_teste.tsv'
    # id_cols = 'macsaud_code'
    # unique_id = 'macsaud_code'
    # extra_cols = ['macsaud', 'regiao', 'coduf', 'estado', 'uf_sigla']
    # add_cols = ''
    # # target = 'test_result'
    # ignore_cols = ['codmun']
    # data_format = 'integer'
    # sortby = ['macsaud_code']
    # filters = '~macsaud_code:'
    ...
    # set new indices
    # for col in id_cols:
    #     if '.0' in df[col].tolist()[0]:
    #         print(df[col])
    df.insert(0, 'unique_id1', '')
    df['unique_id1'] = df[id_cols].astype(str).sum(axis=1)
    if '.0' in str(df['unique_id1'].tolist()[0]):
        df['unique_id1'] = df['unique_id1'].astype(float).astype(int).astype(str)
